...
const CountriesInfo: React.FC<Props> = ({alphaCode}) => {
  ...
  useEffect(() => {
    const getCountryByCode = async () => {
      const promise: {data: APIOneCountry} = await axios<APIOneCountry>(`https://restcountries.com/v2/alpha/${alphaCode}`);
      ...
      if (countryBorder){
        const borderPromise = countryBorder.map(async alphaCode => {
          const axiosResponse = await axios.get(`https://restcountries.com/v2/alpha/${alphaCode}`);
  ...
  return countries && (
    <div>
      <div className='countryHeader'>
        ...
        <img style={{width: '200px'}} src={countries.flag} alt={countries.name}/>
      </div>
